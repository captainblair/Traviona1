          ...
          {isAuthenticated ? (
            <>
              ...
              {canAccessAdmin(user) && (
                <Link
                  to="/admin"
                  className="rounded-full border border-tide/40 px-4 py-2.5 text-sm font-semibold text-tide transition hover:border-tide hover:bg-tide/10"
                >
                  Admin
                </Link>
              )}
          ...
          <div className="flex flex-col px-4 py-3">
            {isAuthenticated ? (
              <div className="mb-3 border-b border-white/10 pb-3">
                <p className="text-xs font-bold uppercase tracking-[0.12em] text-white/50">Signed in</p>
            ...
            {isAuthenticated && canAccessAdmin(user) && (
              <Link
                to="/admin"
                className="mt-3 inline-flex items-center justify-center rounded-full border border-tide/40 px-5 py-3 text-sm font-bold text-tide transition hover:bg-tide/10"
                onClick={closeMenu}
              >
                Admin console
              </Link>
            ...
            {isAuthenticated ? (
              <button
                ...
                className="mt-3 inline-flex w-full items-center justify-center rounded-full border border-white/20 px-5 py-3 text-sm font-bold text-white transition hover:border-tide hover:text-tide"
              >
                Sign Out
              </button>
            ) : null}
